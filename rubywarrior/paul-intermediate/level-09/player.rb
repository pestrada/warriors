#RubyWarrior - intermediate - Level 09
class Player
  ...
  def play_turn(warrior)
    ...
    if surrounded? then
      if @next_direction == :forward then
        ...
        if @warrior.feel(@next_direction).captive? then
          rescue! @next_direction
          finish_turn = true
        else
          @warrior.bind! @next_direction
          finish_turn = true
        end
      end
    end
    ...
    surroundings.each do |s|
      if s.captive? && s.ticking? then
        ...
        if low_health? && surrounded? then
          ...
          finish_turn = true
          break
        elsif low_health? && !surrounded? then
          if enemy_in_direction? :forward then
            run_away!
            finish_turn = true
            break
          else
  ...
  def enemies_ahead?(direction)
    ...
    elsif units[1] == 's' then
      if units[2] == 's' then return true
      else return false
      end
    end
  end
